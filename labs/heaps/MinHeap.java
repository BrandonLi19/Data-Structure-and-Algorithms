...
public class MinHeap<T extends Comparable<T>> implements PriorityQueue<T> {
	...
	void moveItem(int from, int to) {
		Decreaser<T> a;
		// Save the original from array
		a = array[from];

		// Set from to to array
		array[from] = array[to];
	...
	private void heapify(int where) {
		...
		if(right <= size) {
			...
			T leftvalue = array[left].getValue();
			T rightvalue = array[right].getValue();
			ticker.tick(3);
			
			if(parent.compareTo(leftvalue)<=0&&parent.compareTo(rightvalue)<=0) {
				ticker.tick();
				return;
			}
			...
			if((leftvalue.compareTo(parent)<0)) {
				moveItem(left,where);
				heapify(left);
				ticker.tick(3);
			}
